...
static const CGFloat CSToastHorizontalPadding   = 30.0;//10.0
...
//color background
#define kBackGroundColour [UIColor colorWithRed:0 green:0 blue:0 alpha:0.8]
...
- (void)makeToast:(NSString *)message {
    [self makeToast:message duration:CSToastDefaultDuration position:CSToastDefaultPosition finishBlock:^{
        
    }];
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position finishBlock:(void (^)())finish{
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    [self showToast:toast duration:interval position:position finishBlock:finish];
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position title:(NSString *)title {
    UIView *toast = [self viewForMessage:message title:title image:nil];
    [self showToast:toast duration:interval position:position finishBlock:^{
...
- (void)showToast:(UIView *)toast {
    [self showToast:toast duration:CSToastDefaultDuration position:CSToastDefaultPosition finishBlock:^{
        
    }];
}

- (void)showToast:(UIView *)toast duration:(CGFloat)interval position:(id)point finishBlock:(void (^)())finish{
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (title != nil) {
        ...
        if ([[[UIDevice currentDevice] systemVersion]floatValue] >= 7.0) {
            expectedSizeTitle = [title boundingRectWithSize:maxSizeTitle options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
        }else{
//            expectedSizeTitle = [title sizeWithFont:titleLabel.font constrainedToSize:maxSizeTitle lineBreakMode:titleLabel.lineBreakMode];//ios6有效
...
//        CGSize expectedSizeMessage = [message sizeWithFont:messageLabel.font constrainedToSize:maxSizeMessage lineBreakMode:messageLabel.lineBreakMode];//此方法过期

        NSDictionary *attributes = @{NSFontAttributeName:messageLabel.font};
        CGSize expectedSizeMessage;
        if ([[[UIDevice currentDevice] systemVersion]floatValue] >= 7.0) {
expectedSizeMessage = [message boundingRectWithSize:maxSizeMessage options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
        }else{
//            expectedSizeMessage = [message sizeWithFont:messageLabel.font constrainedToSize:maxSizeMessage lineBreakMode:messageLabel.lineBreakMode];// iOS6有效

        }
        messageLabel.frame = CGRectMake(0.0, 0.0, expectedSizeMessage.width, expectedSizeMessage.height);
//        NSLog(@"-------:%f",expectedSizeMessage.width);
    }
